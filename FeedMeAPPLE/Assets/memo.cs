...
public class memo : MonoBehaviour
{
    ...
    private void InferMapFromQTable()
    {
        ...
        {
            Debug.Log("InferMapFromQTable: qTable が空です");
            return;
        }

        // 閾値（必要ならチューニング）
        const float holeThreshold = -300f;
        const float goalThreshold = 300f;

        // ゴール候補は「最大の maxQ」を採用する
        float bestGoalQ = float.NegativeInfinity;
        Vector2Int bestGoalPos = new Vector2Int(0, 0);
        ...
        foreach (var kv in qTable)
        {
            ...
            if (string.IsNullOrEmpty(stateKey) || qrow == null) continue;

            // Q行のインデックス 1..ACTION_SPACE を使う（index0は未使用）
            int maxIndex = Mathf.Min(ACTION_SPACE, qrow.Count - 1);
            if (maxIndex < 1) continue;

            float maxQ = float.NegativeInfinity;
            float minQ = float.PositiveInfinity;
            ...
            // stateKey を "(x, y, env, up, down, right, left)" の形式からパース
            try
            {
                ...
                if (parts.Length >= 2)
                {
                    ...
                    // 穴判定（どれかのアクションで極端に負）
                    if (minQ < holeThreshold)
                    {
                        estimatedHoles.Add(pos);
                    }

                    // ゴール候補（maxQ が閾値を超えるなら候補になりうる）
                    if (maxQ > goalThreshold)
                    {
                        // より大きな maxQ を持つ位置を優先して保存
                        if (maxQ > bestGoalQ)
                        {
                            bestGoalQ = maxQ;
                            bestGoalPos = pos;
                ...
                {
                    Debug.LogWarning($"InferMapFromQTable: stateKey の parts が不正です: '{stateKey}'");
                }
            }
            catch (Exception e)
            {
                Debug.LogWarning($"InferMapFromQTable: stateKey のパースに失敗しました: '{stateKey}' -> {e.Message}");
                continue;
            }
        }
        ...
        Debug.Log($"InferMapFromQTable 完了 — 推定穴数={estimatedHoles.Count}, 推定ゴール={(estimatedGoal.HasValue ? estimatedGoal.ToString() : "未検出")}");
    }
